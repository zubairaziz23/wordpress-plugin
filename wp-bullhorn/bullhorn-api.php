...
define('BH_REST_URL_CUSTOM','https://rest.bullhornstaffing.com');
define('BH_LOGIN',BH_REST_URL_CUSTOM . '/rest-services/login?version=*&access_token=%s');
...
class BullhornAPI {
	...
	$url = sprintf(BH_ROOT_URL . "/oauth/authorize?client_id=%s&response_type=code&username=eos.api&password=Dawaam1234!&action=Login", self::$client_id);
	$ch = curl_init();
...
//print_r($a);exit;
	...
	public function getJobsAndInsert($start,$per_page) {
		$uri = $this->rest_uri ."search/JobOrder?fields=id,businessSectors,title,address,owner,description,dateAdded,categories&count=".$per_page."&start=".$start."&query=isOpen:1&orderBy=dateAdded-&BhRestToken=" . $this->rest_token;
        $response = \Httpful\Request::get($uri)->send();
		//print_r($response);exit;
		return json_encode($response->body);
    }
...
function BHGetCategoryData($entity_type, $entity_id) {
	...
	public function parseCandidate($ofile) {
		...
		$ext = $ofile['ext'];
		$file = $ofile['file'];
		$uri = $this->rest_uri ."resume/parseToCandidate?format=$ext&populateDescription=html&BhRestToken=" . $this->rest_token;
	...
	public function processCandidate($body, $ofile,$data,$bhId) {
		...
		if(!empty($bizsectors)) {
			...
			$response = \Httpful\Request::put($uri)->send();
			//error_log('businesssectors response:'.$response);
			if(empty($response->body->changedEntityId)) {
				//error_log('businesssectors:'.$uri);
